/**
 * This code was generated by v0 by Vercel.
 * @see https://v0.dev/t/VaAwnXY6OEC
 */

import { JSX, SVGProps } from "react";
import { ModeToggle } from "./toogle/theme";
import { Card, CardContent } from "./ui/card";

export function UnderConstruction() {
  return (
    <Card className="mx-auto w-fit  flex flex-col items-center justify-center border border-primary ">
      <CardContent>
        <div className="flex flex-col items-center justify-center    gap-2 text-center">
          <div className="w-full justify-end flex flex-row gap-2 items-center">
            Theme
            <ModeToggle />
          </div>
          <div className="space-y-2">
            <ConstructionIcon className="mx-auto h-12 w-12" />
            <h1 className="text-3xl font-bold tracking-tighter sm:text-4xl md:text-5xl">
              Under Construction
            </h1>
            <p className="mx-auto max-w-[500px] text-gray-500 md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed dark:text-gray-400">
              I am working on something awesome! Please Check back soon.
            </p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 justify-center">
            <div className="flex flex-col items-center gap-1">
              <span className="text-sm font-medium tracking-wide text-gray-500 md:text-base dark:text-gray-400">
                Days
              </span>
              <span className="text-xl font-semibold md:text-2xl">42</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-sm font-medium tracking-wide text-gray-500 md:text-base dark:text-gray-400">
                Hours
              </span>
              <span className="text-xl font-semibold md:text-2xl">10</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-sm font-medium tracking-wide text-gray-500 md:text-base dark:text-gray-400">
                Minutes
              </span>
              <span className="text-xl font-semibold md:text-2xl">23</span>
            </div>
            <div className="flex flex-col items-center gap-1">
              <span className="text-sm font-medium tracking-wide text-gray-500 md:text-base dark:text-gray-400">
                Seconds
              </span>
              <span className="text-xl font-semibold md:text-2xl">09</span>
            </div>
          </div>
          <div className="max-w-[600px] w-full grid gap-2 justify-center">
            <div className="flex items-start gap-4">
              <CheckIcon className="h-4 w-4 flex-shrink-0 text-gray-500 dark:text-gray-400" />
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Website design and layout completed.
              </p>
            </div>
            <div className="flex items-start gap-4">
              <CheckIcon className="h-4 w-4 flex-shrink-0 text-gray-500 dark:text-gray-400" />
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Content and media integration in progress.
              </p>
            </div>
            <div className="flex items-start gap-4">
              <CheckIcon className="h-4 w-4 flex-shrink-0 text-gray-500 dark:text-gray-400" />
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Final testing and optimization coming up next.
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            <a
              className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-50"
              href="#"
            >
              <TwitterIcon className="h-6 w-6" />
            </a>
            <a
              className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-50"
              href="#"
            >
              <FacebookIcon className="h-6 w-6" />
            </a>
            <a
              className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-50"
              href="#"
            >
              <InstagramIcon className="h-6 w-6" />
            </a>
            <a
              className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-50"
              href="#"
            >
              <GithubIcon className="h-6 w-6" />
            </a>
          </div>
        </div>
      </CardContent>
    </Card>
  );
}

function ConstructionIcon(
  props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>
) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <rect x="2" y="6" width="20" height="8" rx="1" />
      <path d="M17 14v7" />
      <path d="M7 14v7" />
      <path d="M17 3v3" />
      <path d="M7 3v3" />
      <path d="M10 14 2.3 6.3" />
      <path d="m14 6 7.7 7.7" />
      <path d="m8 6 8 8" />
    </svg>
  );
}

function CheckIcon(props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <polyline points="20 6 9 17 4 12" />
    </svg>
  );
}

function TwitterIcon(props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
    </svg>
  );
}

function FacebookIcon(
  props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>
) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" />
    </svg>
  );
}

function InstagramIcon(
  props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>
) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <rect width="20" height="20" x="2" y="2" rx="5" ry="5" />
      <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
      <line x1="17.5" x2="17.51" y1="6.5" y2="6.5" />
    </svg>
  );
}

function GithubIcon(props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
      <path d="M9 18c-4.51 2-5-2-7-2" />
    </svg>
  );
}
